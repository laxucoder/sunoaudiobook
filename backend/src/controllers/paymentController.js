...
exports.createOrder = async (req, res) => {
  try {
    ...
    if (type === 'CONTENT_PURCHASE') {
      ...
      if (!playlist) return res.status(404).json({ error: "Playlist/Content not found" });
      if (playlist.isFree) return res.status(400).json({ error: "Content is free" });
...
exports.cancelSubscription = async (req, res) => {
  try {
    ...
    if (!user.isAutoRenewal || !user.razorpaySubscriptionId) {
      return res.status(400).json({ msg: "No active auto-debit subscription found." });
    }
    ...
    res.json({ success: true, msg: "Subscription cancelled. Access remains until expiry." });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};